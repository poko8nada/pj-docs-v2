...
function readGateRules() {
  return [
    'Phase: default `discussion`. Hands-on after user `/work` or `/chore`. Session focus → Read `scope/SKILL.md` (`unlock.scope`; `/discussion` closes). Edit → Read `rules/SKILL.md` (`unlock.rules`). Issue writes only in `/work` (not code/findings alone): Read `issue/SKILL.md` + matching Goal/Discover/Build template (`unlock.issue`; template in `read.refs` as `issue/<template>.md`). Phase re-entry clears `read.skills` / `read.refs` (`unlock.scope` stays open until `/discussion`). Broken → user `/bootstrap` only.',
