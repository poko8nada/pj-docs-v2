...
function readGateRules() {
  return [
    ...
    '## Review',
    '',
    '- Reviewable current Git snapshot differs from the bound review snapshot → `git commit` blocked.',
    '- Clear: `/pre-commit-reviewer` → `REVIEW: PASS` → bind the matching snapshot, then stop or `git commit`.',
    '- Reviewer child IDs are bound only after a unique post-snapshot PASS matches the parent prompt read from the sticky state ID; runtime transcript paths are directory fallback only.',
    '- Only current Git changes with the existing code/CSS/HTML extension filter enter the snapshot.',
    '',
    '## State files',
    '',
