...
description: >-
  ...
  After discussion agrees the session scope: use plan (inventory → slices → user agreement), then execute and commit.
...
Product cuts live in the **`plan` skill** — inventory, slice table, **user agreement before execute**. Do not copy that content here.

**Harness:** Read `.cursor/skills/plan/SKILL.md` sets `unlock.plan: true` (work only). Edits need `unlock.scope` → `unlock.plan` → `unlock.rules`. `/chore` does not use `unlock.plan` (`null`).

**Agreement:** After presenting the plan (or the next slice), stop and wait for the user. Do not implement in the same turn as the first plan dump.
...
```text
scope → plan (inventory → slices → user agrees) → Execute → Commit
...
**Handshake:** Session focus → Read `.cursor/skills/scope/SKILL.md` if `scope` is not open. Plan → Read `.cursor/skills/plan/SKILL.md` (and get agreement before execute). File edits → Read `.cursor/skills/rules/SKILL.md`, then a matching `rules/references/*`. Issue create/update → Read `.cursor/skills/issue/SKILL.md`, then `goal` / `discover` / `build` template.

Inspect open `[Goal]` / `[Discover]` / `[Build]` and `findings/` as needed.

- **Scope already agreed** (discussion opened scope) → Read `plan` and start inventory / agreed slice; no ceremony on Theme.
...
- Do not skip plan → user agreement before execute.
- Do not treat Build issue as a durable Plan of slices — session slices stay in chat via `plan`.

Hand off to `scope` / `plan` / `issue` / `rules` / soft skills by name — do not copy their contents here.
