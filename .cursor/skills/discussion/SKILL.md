...
name: discussion
description: >-
  Default phase and user-controlled return hatch from hands-on. Establish focus, then await `/scope ok`.
  Use when starting a conversation, stepping back from hands-on, or when focus must be re-agreed.
  `/discussion` clears unlock.scope and label.
disable-model-invocation: true
---

# discussion

Establish a provisional session focus with `scope`, then let the user confirm it with
`/scope ok`. Prose and reading `scope` do not open `unlock.scope`.
...
4. Keep talking on that focus, or let the user invoke `/work` / `/chore` after confirmation. Do not stop only to wait for `/work`. Do not self-invoke phases.
