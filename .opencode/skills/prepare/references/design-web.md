...
The design spec lives in the [Design] issue body — not in a local file. The body is written **in one shot at run stage 6** (`Update the [Design] issue body`). Stages 1–5 of the run (web type confirmation, sections + components, default screen, component comments, discuss/iterate) do not touch the body — the prototype and the chat discussion are the working surface during those stages.
